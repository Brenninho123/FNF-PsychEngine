...
{
	public static var psychEngineVersion:String = '1.0.5';
	...
	function getActiveItem():FlxSprite
	{
	...
	function selectItem()
	{
		...
		var item:FlxSprite = getActiveItem();
		var option:String = switch (curColumn)
		{
			case CENTER: optionShit[curSelected];
		...
		{
			handleMenuSelection(option, item);
		});
		
		for (memb in menuItems.members)
		{
			if (memb != null && memb != item)
				FlxTween.tween(memb, {alpha: 0}, 0.4, {ease: FlxEase.quadOut});
		}
	}

	function handleMenuSelection(option:String, item:FlxSprite)
	{
		switch (option)
		{
			case 'story_mode':
